@php
    ...
    if (is_plugin_active('blog')) {
        ...
        $topNewsData = $getPostsForCategory(28, 6);
        $nationalData = $getPostsForCategory(12, 4);
        $countrywideData = $getPostsForCategory(9, 4); 
        $sportsData = $getPostsForCategory(24, 7); 
        $entertainmentData = $getPostsForCategory(15, 7);
        $economyData = $getPostsForCategory(14, 4); // 
        $politicsData = $getPostsForCategory(16, 4);
        $lifestyleData = $getPostsForCategory(17, 4);
        $jobsData = $getPostsForCategory(21, 4);
        // Data for right columns or special sections
        $opinionData = $getPostsForCategory(9, 1); // *** Change 20 to your 'মতামত' category ID ***
        $politicsRightData = $getPostsForCategory(13, 3); 
       ...
       <!-- topnews -->
            ...
            <div class="row">
                                ...
                                <div class="d-flex flex-column gap-3 border-bottom">
                                    <div class="card news-card-sm">
                                        <div class="card-body d-flex gap-3 align-items-center">
                                            ...
                                            <img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $post->name }}" class="lazyload img-fluid">
                                        </div>
                                    </div>
                                </div>
                            </a>
                ...
                <div class="col-md-3">
                    ...
                    <div class="opinion-section mt-3">
                        <h4>{{ $opinionData->category->name }}</h4>
                         @foreach ($opinionData->posts as $post)
                        <a href="{{ $post->url }} ">
                            <div class="opinion-item d-flex align-items-center gap-3">
                                <img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}" class="-circle lazyload" alt="{{ $post->name }} ">
                                <div>
                                    <h6>{{ $post->name }} </h6>
                                </div>
                            </div>
                            ...
                            <div class="bg-cat-news card mb-3">
                                <a href="{{ $firstPost->url }}">
                                    <div class="cate-two-zoom5">
                                        <img  data-src="{{ RvMedia::getImageUrl($firstPost->image, 'featured', false, RvMedia::getDefaultImage()) }}" alt="{{ $firstPost->name }}" class="img-fluid lazyload" >
                                    </div>
        ...
        <section class="cat-two">
            <div class="cat-two-area my-4">
                <div class="row g-4">
                    <div class="opinion-section mt-3">
                        <h4><a href="{{ $countrywideData->category->url }}">{{ $countrywideData->category->name }} </a></h4>
                    </div>
                    <div class="col-lg-9 border-right">
                        @php $firstPost = $countrywideData->posts->first(); @endphp
                        @if ($firstPost)
                        <div class="card ">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <a href="{{ $firstPost->url }}" class="d-block cate-two-zoom-big">
                                        <img data-src="{{ RvMedia::getImageUrl($firstPost->image, 'featured', false, RvMedia::getDefaultImage()) }}" alt="{{ $firstPost->name }}"  class="lazyload img-fluid">
                                    </a>
                                </div>
                                <div class="col-md-6 d-flex flex-column justify-content-center">
                                    ...
                                    <p class="text-muted small">{{ Str::limit($firstPost->description, 130) }}</p>
                                </div>
                            </div>
                        </div>
                        @endif
                        <div class="row g-4 mt-3">
                            @foreach ($countrywideData->posts->skip(1) as $post)
        ...
        <section class="cat-four">
            <div class="row">
                @php
                    $firstPost = $sportsData->posts->get(0);
                    $middlePosts = $sportsData->posts->slice(1, 3);
                    $rightPosts = $sportsData->posts->slice(4, 3);
                @endphp
                <div class="opinion-section mt-3">
                    <h4><a href="{{ $sportsData->category->url }}">{{ $sportsData->category->name }} </a> </h4>
                </div>
                       ...
                       @foreach ($middlePosts as $post)
                        <div class="card news-card border-bottom pb-3 img-hover-zoom">
                            <div class="card-body d-flex gap-3 ">
                                <div class="flex-grow-1"><a href="{{ $post->url }}"><h5 class="card-title">{{ $post->name }}</h5></a></div>
                                <a href="{{ $post->url }}"><div class="cate-two-zoom6"><img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}"  alt="{{ $post->name }}" class="lazyload img-fluid"></div></a>
                            </div>
                        </div>
                        @endforeach
                    </div>
        ...
        <section class="cat-four">
            <div class="row">
                @php
                    $firstPostCol = $entertainmentData->posts->slice(0, 3);
                    $middlePostCol = $entertainmentData->posts->slice(3, 1)->first();
                    $rightPostCol = $entertainmentData->posts->slice(4, 3);
                @endphp
                <div class="opinion-section mt-3">
                    <h4><a href="{{ $entertainmentData->category->url }}">{{ $entertainmentData->category->name }} </a> </h4>
                </div>
                <div class="col-lg-4 border-right">
                    <div class="d-flex flex-column gap-3">
                        @foreach($firstPostCol as $post)
                        <div class="card news-card border-bottom pb-3 img-hover-zoom">
                            <div class="card-body d-flex gap-3 ">
                                <div class="flex-grow-1"><a href="{{ $post->url }}"><h5 class="card-title">{{ $post->name }}</h5></a></div>
                                <a href="{{ $post->url }}"><div class="cate-two-zoom6"><img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}"  alt="{{ $post->name }}" class="lazyload img-fluid"></div></a>
                 ...
                 <div class="col-lg-4 border-right">
                     @if ($middlePostCol)
                        <div class="bg-cat-news card mb-3">
                            <a href="{{ $middlePostCol->url }}">
                                <div class="cate-two-zoom5">
                                    <img data-src="{{ RvMedia::getImageUrl($middlePostCol->image, 'featured', false, RvMedia::getDefaultImage()) }}"  alt="{{ $middlePostCol->name }}" class="lazyload img-fluid">
                                </div>
                                <h4 class="pt-3"> {{ $middlePostCol->name }} </h4>
                                <p class="mt-2">{{ Str::limit($middlePostCol->description, 130) }}</p>
                            </a>
                        </div>
                     @endif
                </div>
        ...
        <!-- অর্থনীতি -->
        @if ($economyData->category && $economyData->posts->isNotEmpty())
        <section class="cat-two">
            <div class="cat-two-area my-4">
                <div class="opinion-section mt-3">
                    <h4><a href="{{ $economyData->category->url }}">{{ $economyData->category->name }} </a> </h4>
                </div>
                <div class="row g-4 mt-3">
                    @foreach ($economyData->posts as $post)
                    <div class="col-sm-6 col-6 col-md-3 border-right">
                        <a href="{{ $post->url }}" class="text-decoration-none text-dark">
                            <div class="card border-0 h-100">
                                <div class="cate-two-zoom">
                                    <img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}"  alt="{{ $post->name }}" class="lazyload img-fluid card-img-top fixed-img-two">
                                </div>
                                <div class="card-body p-2">
                                    <h4 class="news-title-two">{{ $post->name }}</h4>
                                </div>
        ...
        <section class="cat-one">
            <div class="row g-4">
                ...
                <div class="col-lg-4 border-left ">
                    @if ($politicsRightData->category && $politicsRightData->posts->isNotEmpty())
                    <div class="opinion-section mt-3">
                        <h4><a href="{{ $politicsRightData->category->url }}">{{ $politicsRightData->category->name }} </a> </h4>
                    </div>
                    <div class="d-flex flex-column gap-3">
                        @foreach ($politicsRightData->posts as $post)
                        <div class="card news-card border-bottom pb-3 img-hover-zoom">
                            <div class="card-body d-flex gap-3 ">
                                <div class="flex-grow-1"><a href="{{ $post->url }}"><h5 class="card-title">{{ $post->name }}</h5></a></div>
                                <a href="{{ $post->url }}"><div class="cate-two-zoom6"><img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}"  alt="{{ $post->name }}" class="lazyload img-fluid"></div></a>
        ...
        <!-- লাইফস্টাইল -->
        @if ($lifestyleData->category && $lifestyleData->posts->isNotEmpty())
        <section class="cat-two">
            <div class="cat-two-area my-4">
                <div class="opinion-section mt-3">
                    <h4><a href="{{ $lifestyleData->category->url }}">{{ $lifestyleData->category->name }} </a> </h4>
                </div>
                <div class="row g-4 mt-3">
                    @foreach ($lifestyleData->posts as $post)
                    <div class="col-sm-6 col-6 col-md-3 border-right">
                        <a href="{{ $post->url }}" class="text-decoration-none text-dark">
                            <div class="card border-0 h-100">
                                <div class="cate-two-zoom">
        ...
        <!-- জবস -->
        @if ($jobsData->category && $jobsData->posts->isNotEmpty())
        <section class="cat-two">
            <div class="cat-two-area my-4">
                <div class="row g-4">
                    ...
                    <div class="col-lg-8 border-right">
                        <div class="opinion-section mt-3">
                       <h4><a href="{{ $jobsData->category->url }}">{{ $jobsData->category->name }} </a> </h4>
                    </div>
                        @php $firstPost = $jobsData->posts->first(); @endphp
                        @if ($firstPost)
                        <div class="card ">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <a href="{{ $firstPost->url }}" class="d-block cate-two-zoom-big">
                                        <img data-src="{{ RvMedia::getImageUrl($firstPost->image, 'featured', false, RvMedia::getDefaultImage()) }}"  alt="{{ $firstPost->name }}" class="lazyload img-fluid w-100">
                                    </a>
                                </div>
                                <div class="col-md-6 d-flex flex-column justify-content-center">
                                    <a href="{{ $firstPost->url }}" class="text-decoration-none text-dark">
                        ...
                        @endif
                        <div class="row g-4 mt-3">
                            @foreach ($jobsData->posts->skip(1) as $post)
                            <div class="col-sm-6 col-6 col-md-4 border-right">
                                <a href="{{ $post->url }}" class="text-decoration-none text-dark">
                                    <div class="card border-0 h-100">
                                        <div class="cate-two-zoom">
                    ...
                    <div class="col-lg-4">
                        @if ($jobsRightData->category && $jobsRightData->posts->isNotEmpty())
                        <div class="opinion-section mt-3">
                            <h4><a href="{{ $jobsRightData->category->url }}">{{ $jobsRightData->category->name }} </a> </h4>
                        </div>
                        <div class="d-flex flex-column gap-3">
                            @foreach ($jobsRightData->posts as $post)
                                <div class="card news-card border-bottom pb-3 img-hover-zoom">
                                    <div class="card-body d-flex gap-3 ">
                                        <div class="flex-grow-1"><a href="{{ $post->url }}"><h5 class="card-title">{{ $post->name }}</h5></a></div>
                                        <a href="{{ $post->url }}"><div class="cate-two-zoom6"><img data-src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}"  alt="{{ $post->name }}" class="lazyload img-fluid"></div></a>
